...
/**
* Утилиты для удобной работы с OpenCL.
*/
...
/**
* Информация о поддержке оборудованием стандарта OpenCL.
*
...
*/
void printCLInfo( cl_platform_id platform );
void printCLInfo();




inline void printCLInfo() {

    static const size_t MAX_PLATFORMS = 16;

    cl_platform_id  platforms[ MAX_PLATFORMS ];
    cl_uint numPlatforms;

    if (clGetPlatformIDs( MAX_PLATFORMS, platforms, &numPlatforms ) != CL_SUCCESS) {
        fprintf( stderr, "Unable to get OpenCL platforms\n." );
        return;
    }

    for (cl_uint i = 0; i < numPlatforms; i++) {
        printCLInfo( platforms[ i ] );
    }
}
...
inline void printCLInfo( cl_platform_id platform ) {
    ...
    clGetPlatformInfo(platform, CL_PLATFORM_PROFILE,sizeof(param),param,&ret_size);
    printf("\nPlatform Profile:\t%s\n",param);
    clGetPlatformInfo(platform, CL_PLATFORM_VERSION,sizeof(param),param,&ret_size);
    printf("Platform Version:\t%s\n",param);

    clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL,MAX_DEVICES,devices,&num_devices);
